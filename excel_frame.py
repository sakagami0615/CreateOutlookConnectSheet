...
import utility
from excel_value import ExcelValue
from export_frame import ConnectValue


class SelectExcelFrameWidget:
	...
	def __init__(self, parent):		
		...
		# Create Combobox
		self.select_sheet = tkinter.ttk.Combobox(self.frame_select, state='readonly')
		self.select_sheet.grid(row=1, column=1, padx=10, ipadx=20)
		# --------------------------------------------------------------------------------
...
class SelectExcelFrame(tkinter.ttk.Frame):
	...
	def __init__(self, master, tab_name, tab_index, width, height):
		...
		# Init Window BEGIN
		# --------------------------------------------------------------------------------
		self.widget = SelectExcelFrameWidget(self)
	...
	def GetContentList(self):
		...
		if not self.excel_object: return False, None

		sheet_value = self.excel_object.GetValue(self.widget.select_sheet.get())
		row_size = self.excel_object.GetRowNum(self.widget.select_sheet.get())
	...
	def GetLastnameLine(self):
		...
		col_size = self.excel_object.GetColNum(self.widget.select_sheet.get())
	...
	def ButtonClick_SelectFile(self):
		...
		self.excel_object = ExcelValue(file_name)
		self.widget.select_sheet['values'] = self.excel_object.sheet_names
		self.widget.select_sheet.current(0)
		
	
	def ButtonClick_View(self):
		
		if not self.excel_object: return
